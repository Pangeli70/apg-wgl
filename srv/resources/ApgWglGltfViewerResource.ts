...
export class ApgWglGltfViewerResource extends Edr.ApgEdrLoggableResource {
  ...
  public async GET(request: Edr.Drash.Request, response: Edr.Drash.Response) {
    ...
    const rawGltf = decodeURI(request.pathParam("gltf")!);


    const gltfFile = Uts.Std.Path.resolve(Wgl.ApgWglTestService.TestOutputPath + rawGltf);
