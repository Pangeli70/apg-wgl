...
import { ApgWglProfileBuilder } from './ApgWglProfileBuilder.ts'
import { ApgWglPathBuilder } from './ApgWglPathBuilder.ts'


export class ApgWglExtrusionBuilder {


  public static extrudeRandomShape() {
    ...
    const shapePoints = (Math.random() > 0.5) ?
      ApgWglProfileBuilder.buildRandomStarProfile(30, 25) :
      ApgWglProfileBuilder.buildRandomPolygonProfile(20);
  ...
  public static extrudeRandomShapeAlongRandomOpenedPolyLine(
    aoptions: IApgWglPolylineExtrusionSettings
  ) {

    const polyLine = ApgWglPathBuilder.buildRandomizedXYPolyline(
      aoptions.nodes,
  ...
  public static extrudeRandomShapeAlongRandomOpenedSpline() {
    const openedSpline = ApgWglPathBuilder.buildRandomizedOpenedSplinePath();
  ...
  public static extrudeRandomShapeAlongRandomClosedSpline() {
    const edges = Math.round(Math.random() * 9) + 6; // always even
    const closedSpline = ApgWglPathBuilder.buildRandomizedStarShapeClosedSplinePath(edges);
